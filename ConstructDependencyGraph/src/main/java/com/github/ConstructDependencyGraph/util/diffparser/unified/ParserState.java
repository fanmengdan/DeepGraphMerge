 ...
 */
package com.github.ConstructDependencyGraph.util.diffparser.unified;

import org.apache.log4j.Logger;

import static com.github.ConstructDependencyGraph.util.diffparser.api.UnifiedDiffParser.LINE_RANGE_PATTERN;
...
//        return NEUTRAL_LINE;
  ...
  protected boolean matchesEndPattern(String line, ParseWindow window) {
    if ("".equals(line.trim())) {
      // We have a newline which might be the delimiter between two diffs. It may just be an empty
      // line in the current diff or it
      // may be the delimiter to the next diff. This has to be disambiguated...
      int i = 1;
      String futureLine;
      while ((futureLine = window.getFutureLine(i)) != null) {
        if (matchesFromFilePattern(futureLine, window.getFutureLine(1))) {
          // We found the start of a new diff without another newline in between. That makes the
          // current line the delimiter
          // between this diff and the next.
          return true;
        } else if ("".equals(futureLine.trim())) {
          // We found another newline after the current newline without a start of a new diff in
          // between. That makes the
      ...
      if (nextFromFileLine != null
          && matchesFromFilePattern(nextFromFileLine, window.getFutureLine(1))) {
        window.addLine(1, "");
